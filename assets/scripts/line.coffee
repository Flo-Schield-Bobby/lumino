###
## Inspired by the open-source project : [TimelineJS][1]
## [1]: https://github.com/russellmcc/timelinejs
###
...
$.fn.line = (m) ->
  ...
  methods =
    init : (o) ->
      $t = $ @
      if ($t.data 'line')?
        ret = ($t.data 'line') unless found
        found = yes
      else
        t = new Line @, o if $t.is 'div'
        $t.data 'line', t
        ret = t unless found
        found = yes
    resize: (w, h) ->
  ...
  @each ->
    if methods[m]
      ...
      $.error "Method #{method} does not exist on jQuery.line"
  return ret
  return
